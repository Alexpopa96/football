...
class Store extends Controller
{
    ...
    {
        abort_unless($friendly->status === 'live', 422, 'Poți paria doar pe meciuri live.');
        abort_if($friendly->betting_locked_at, 422, 'Pariurile au fost blocate pentru acest meci.');
        abort_if(in_array(Auth::id(), $friendly->participantUserIds(), true), 403, 'Nu poți paria pe propriul meci.');
        ...
        $user = Auth::user();
        $existing = $friendly->bets()->where('user_id', $user->id)->first();
        ...
        DB::transaction(function () use ($friendly, $user, $existing, $data, $odds) {
            if ($existing) {
