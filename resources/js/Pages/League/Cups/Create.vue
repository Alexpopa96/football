...
function submit() {
    form.post('/league/cups/store');
}
</script>

<template>
    <LeagueLayout title="Cupă nouă">
        <div class="mx-auto max-w-2xl">
            <h1 class="mb-1 font-display text-3xl font-extrabold tracking-tight text-white">Cupă nouă</h1>
            <p class="mb-2 text-slate-400">Dă-i un nume și alege 3 sau 4 jucători.</p>
            <p class="mb-8 text-xs text-slate-500">Ordinea contează: primii doi aleși se întâlnesc într-o semifinală, următorii doi într-o altă semifinală (sau, la 3 jucători, al treilea primește calificare directă în finală).</p>

            <form class="space-y-6" @submit.prevent="submit">
                <div>
                    <label class="text-xs font-medium text-slate-300">Numele cupei</label>
                    <input v-model="form.name" type="text" placeholder="ex: Cupa de vară" class="mt-1 w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-slate-500 focus:border-emerald-400 focus:ring-emerald-400" />
                ...
                <div>
                    ...
                    <div class="mt-2 grid grid-cols-2 gap-3 sm:grid-cols-4">
                        ...
                        >
                            ...
                            <span v-if="form.player_ids.includes(player.id)" class="text-[11px] font-semibold text-emerald-400">#{{ form.player_ids.indexOf(player.id) + 1 }}</span>
                        </button>
                    </div>
