...
const poolForm = useForm({
    team_ids: [...(props.championship.wheel_pool_team_ids || [])],
});

const takenTeamIds = computed(() => props.championship.entries.filter((e) => e.team_id).map((e) => e.team_id));

const remainingPoolTeams = computed(() =>
    props.teams.filter((t) => (poolForm.team_ids || []).includes(t.id) && !takenTeamIds.value.includes(t.id))
);

const allAssigned = computed(() => props.championship.entries.every((e) => !!e.team_id));

const unassignedEntries = computed(() => props.championship.entries.filter((e) => !e.team_id));
...
watch(
    ...
    ([unassigned, pool]) => {
        ...
        router.put(
            `/league/championships/${props.championship.id}/entries/${entry.id}/assign`,
            { team_id: team.id },
            {
                preserveScroll: true,
...
function savePool() {
    poolForm.put(`/league/championships/${props.championship.id}/wheel-pool`, { preserveScroll: true });
}
...
function pickManual(team) {
    router.put(`/league/championships/${props.championship.id}/entries/${manualEntry.value.id}/assign`, { team_id: team.id }, {
        preserveScroll: true,
...
async function requestSpin() {
    ...
    wheelWinner.value = null;
    try {
        const { data } = await window.axios.post(`/league/championships/${props.championship.id}/entries/${wheelEntry.value.id}/spin`);
...
function generate() {
    router.post(`/league/championships/${props.championship.id}/generate`);
}
</script>

<template>
    <LeagueLayout title="Selectare echipe">
        <h1 class="mb-1 font-display text-2xl font-extrabold tracking-tight text-white sm:text-3xl">{{ championship.name }}</h1>
        ...
        <!-- Entries -->
        <div class="space-y-3">
            <div v-for="entry in championship.entries" :key="entry.id" class="flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/5 p-5 backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between">
